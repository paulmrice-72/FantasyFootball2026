...
gh api repos/$REPO/milestones -f title="Phase 1 — Platform Foundation"    -f description="Solution scaffold, databases, CQRS, auth, Blazor shell, CI/CD" -f state="open" 2>/dev/null
gh api repos/$REPO/milestones -f title="Phase 2 — Data Ingestion"         -f description="Sleeper API, NFL stats, historical data loading" -f state="open" 2>/dev/null
gh api repos/$REPO/milestones -f title="Phase 3 — Weekly Engine"          -f description="Projections, Monte Carlo simulation, lineup optimizer" -f state="open" 2>/dev/null
gh api repos/$REPO/milestones -f title="Phase 4 — Dynasty Engine"         -f description="Aging curves, breakout classifier, trade analyzer" -f state="open" 2>/dev/null
gh api repos/$REPO/milestones -f title="Phase 5 — DFS Builder"            -f description="DFS slate importer, ownership model, multi-lineup export" -f state="open" 2>/dev/null
gh api repos/$REPO/milestones -f title="Phase 6 — Polish & Hardening"     -f description="Alerts, caching, architecture tests, production readiness" -f state="open" 2>/dev/null
...
M1=$(get_milestone "Phase 1")
M2=$(get_milestone "Phase 2")
M3=$(get_milestone "Phase 3")
M4=$(get_milestone "Phase 4")
M5=$(get_milestone "Phase 5")
M6=$(get_milestone "Phase 6")

echo "Milestone IDs: P1=$M1 P2=$M2 P3=$M3 P4=$M4 P5=$M5 P6=$M6"
echo ""
