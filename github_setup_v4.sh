...
**Phase:** 1 | **Priority:** Critical Path" \
  -F milestone=$M1 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"epic\",\"phase-1\",\"backend\",\"infra\"]" --silent
sleep 0.5
...
**PBIs:** PBI-001, PBI-002, PBI-003" \
  -F milestone=$M1 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-1\",\"backend\"]" --silent
sleep 0.5
...
- [ ] Add .editorconfig and .gitignore" \
  -F milestone=$M1 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-1\",\"backend\"]" --silent
sleep 0.5
...
- [ ] Add request logging middleware" \
  -F milestone=$M1 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-1\",\"backend\",\"infra\"]" --silent
sleep 0.5
...
**PBIs:** PBI-004, PBI-005, PBI-006" \
  -F milestone=$M1 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-1\",\"backend\",\"data\"]" --silent
sleep 0.5
...
- [ ] Seed reference data (positions, NFL teams)" \
  -F milestone=$M1 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-1\",\"backend\",\"data\"]" --silent
sleep 0.5
...
**PBIs:** PBI-011, PBI-012" \
  -F milestone=$M1 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-1\",\"frontend\"]" --silent
sleep 0.5
...
- [ ] Configure HttpClient with JWT Authorization header injection" \
  -F milestone=$M1 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-1\",\"frontend\"]" --silent
sleep 0.5
...
**Epic:** E1 — Platform Foundation
**PBIs:** PBI-013" \
  -F milestone=$M1 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-1\",\"infra\"]" --silent
sleep 0.5
...
- [ ] Add branch protection rules on main" \
  -F milestone=$M1 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-1\",\"infra\"]" --silent
sleep 0.5
...
**Phase:** 2" \
  -F milestone=$M2 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"epic\",\"phase-2\",\"data\"]" --silent
sleep 0.5
...
**PBIs:** PBI-014, PBI-015, PBI-016" \
  -F milestone=$M2 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-2\",\"data\",\"backend\"]" --silent
sleep 0.5
...
- [ ] Write contract tests against Sleeper API" \
  -F milestone=$M2 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-2\",\"data\",\"backend\"]" --silent
sleep 0.5
...
**PBIs:** PBI-020" \
  -F milestone=$M2 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-2\",\"data\"]" --silent
sleep 0.5
...
- [ ] Display quality summary dashboard page in Blazor" \
  -F milestone=$M2 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-2\",\"data\",\"frontend\"]" --silent
sleep 0.5
...
**Phase:** 3" \
  -F milestone=$M3 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"epic\",\"phase-3\",\"analytics\",\"backend\"]" --silent
sleep 0.5
...
**PBIs:** PBI-021, PBI-022, PBI-023" \
  -F milestone=$M3 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-3\",\"analytics\",\"backend\"]" --silent
sleep 0.5
...
- [ ] Unit test metric calculations against known game logs" \
  -F milestone=$M3 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-3\",\"analytics\",\"backend\"]" --silent
sleep 0.5
...
**PBIs:** PBI-028, PBI-029" \
  -F milestone=$M3 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-3\",\"frontend\"]" --silent
sleep 0.5
...
- [ ] Export projections to CSV" \
  -F milestone=$M3 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-3\",\"frontend\"]" --silent
sleep 0.5
...
**Phase:** 4" \
  -F milestone=$M4 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"epic\",\"phase-4\",\"analytics\",\"backend\"]" --silent
sleep 0.5
...
**PBIs:** PBI-030, PBI-031" \
  -F milestone=$M4 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-4\",\"analytics\",\"backend\"]" --silent
sleep 0.5
...
- [ ] Store curve coefficients as configuration documents in MongoDB" \
  -F milestone=$M4 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-4\",\"analytics\",\"backend\"]" --silent
sleep 0.5
...
**PBIs:** PBI-035, PBI-036" \
  -F milestone=$M4 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-4\",\"frontend\"]" --silent
sleep 0.5
...
- [ ] Filter by position / team / age range" \
  -F milestone=$M4 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-4\",\"frontend\"]" --silent
sleep 0.5
...
**Phase:** 5" \
  -F milestone=$M5 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"epic\",\"phase-5\",\"analytics\",\"backend\"]" --silent
sleep 0.5
...
**PBIs:** PBI-037, PBI-038" \
  -F milestone=$M5 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-5\",\"data\",\"backend\"]" --silent
sleep 0.5
...
- [ ] Handle multi-position flex slot eligibility" \
  -F milestone=$M5 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-5\",\"data\",\"backend\"]" --silent
sleep 0.5
...
- [ ] Unit test salary constraint enforcement" \
  -F milestone=$M5 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-5\",\"analytics\",\"backend\"]" --silent
sleep 0.5
...
**PBIs:** PBI-039, PBI-040" \
  -F milestone=$M5 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-5\",\"analytics\",\"backend\"]" --silent
sleep 0.5
...
**PBIs:** PBI-041" \
  -F milestone=$M5 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-5\",\"frontend\"]" --silent
sleep 0.5
...
- [ ] Export button generating DK/FD contest upload CSV" \
  -F milestone=$M5 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-5\",\"frontend\"]" --silent
sleep 0.5
...
**Phase:** 6" \
  -F milestone=$M6 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"epic\",\"phase-6\",\"backend\",\"infra\"]" --silent
sleep 0.5
...
**PBIs:** PBI-042, PBI-043" \
  -F milestone=$M6 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-6\",\"backend\",\"frontend\"]" --silent
sleep 0.5
...
- [ ] Push real-time in-app notification via SignalR" \
  -F milestone=$M6 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-6\",\"backend\"]" --silent
sleep 0.5
...
- [ ] Filter by position and minimum projected points threshold" \
  -F milestone=$M6 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-6\",\"analytics\",\"frontend\"]" --silent
sleep 0.5
...
**PBIs:** PBI-044, PBI-045" \
  -F milestone=$M6 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"feature\",\"phase-6\",\"backend\",\"infra\"]" --silent
sleep 0.5
...
- [ ] Benchmark query response times before and after caching" \
  -F milestone=$M6 --jq '.number' | xargs -I{} gh api repos/$REPO/issues/{}/labels -f "labels=[\"pbi\",\"phase-6\",\"backend\",\"infra\"]" --silent
sleep 0.5
